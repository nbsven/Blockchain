...
public abstract class ScriptTransaction implements AutoCloseable {
    ...
    public void testScript(Script inputScript, Script spendingScript,Transaction redemptionTransaction) {
        System.out.println("i: "+inputScript.getChunks());
        System.out.println("o: "+spendingScript.getChunks());
        for(ScriptChunk chunk:spendingScript.getChunks()){

            if (!chunk.isOpCode()) {
                System.out.printf("%s op=",Arrays.toString(chunk.data));
